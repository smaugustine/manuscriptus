...
<h1 class="page-header">Manuscriptus: Codex Help</h1>
<div class="row">
	...
	<div id="add" class="col-sm-9 help-content">
		...
		<div class="alert alert-warning">
			Note that identifiers for manuscripts and corpuses can only contain <b>lowercase Latin alphanumeric characters and dashes</b>. They cannot start or end with a dash. It also has to be <b>unique</b> within Manuscriptus: Codex. It is possible to use your own identifier instead of the suggested one, but must follow those requirements. <b>It is not possible to change an identifier after the corpus or manuscript has been created.</b>
		</div>
	</div>
	...
	<div id="markdown" class="col-sm-9 help-content">
		<h2>Markdown</h2>
		<p>In order to format text in Manuscriptus: Codex it is necessary to use a syntax called Markdown. Manuscriptus: Codex does not use the standard flavours of Markdown (e.g. John Gruber's or GitHub's), but rather a modified flavour that is aimed at the sort of formatting that is more useful for manuscript transcription.</p>
	...
	<div id="errors" class="col-sm-9 help-content">
		...
		<p>Some words are used for certain Manuscriptus: Codex functions. In order for these functions to work properly, the required words are not allowed to be used as identifiers. They may still be used as names.</p>
		<h4>The use of HTML is not permitted.</h4>
		<p>HTML cannot be used in any field in Manuscriptus: Codex. Only the provided Markdown syntax may be used. Its use is limited to the corpus description, manuscript description, line text, and line comments fields. This warning will not prevent the form from being processed, but all HTML within the text will either be stripped or neutralized.</p>
		...
		<p>Namespaces are the first part of the URL after the Manuscriptus: Codex directory. The default namespaces are view, new, edit, compare, import, export, backup, help, faq, and about. Plugins are also able to register their own namespaces. Attempting to open a page with a namespace that is neither one of the default namespaces nor one of the namespaces registered by a plugin results in this error, similar to a 404 error from a website.</p>
	</div>
	<div id="overview" class="col-sm-9 help-content">
		<h2>Overview</h2>
		<p>Manuscriptus: Codex is a web-based application designed for storing and analyzing manuscript corpuses.</p>
		
		<h3>How it works</h3>
		<p>Manuscriptus: Codex has three underlying structures: <b>lines</b>, <b>manuscripts</b>, and <b>corpuses</b>. <b>Lines</b> are numbered lines of formatted text which can also have footnotes and comments. <b>Manuscripts</b> are collections of lines. They have a unique identifier as well as a siglum that is unique to the manuscript's corpus. <b>Corpuses</b> are collections of manuscripts. They also have unique identifiers.
	</div>
</div>
